...
The pipeline includes a built-in demo mode with a comprehensive pre-written
travel plan so you can explore the full report format immediately, without
configuring any API credentials.

```bash
# Install dependencies
pip install -r requirements.txt

# Run and save the report (auto-enables demo mode when no API key is set)
python main.py --output results/trip_december_2027.md
```

A sample output is already saved at
[`results/trip_december_2027.md`](results/trip_december_2027.md).

## Prerequisites

- Python 3.11+
- An **Azure OpenAI** resource _or_ an **OpenAI** API key _(optional – demo
  mode runs without any credentials)_
...
```bash
# Run (auto demo mode when no credentials, live LLM when credentials present)
python main.py --output results/trip_december_2027.md

# Force demo mode explicitly (pre-written output, no API calls)
python main.py --demo --output results/trip_december_2027.md

# Explicitly point at a .env file (enables live LLM mode)
python main.py --env /path/to/.env --output results/trip.md

# Print to terminal instead of saving to a file
python main.py --demo
```
...
In demo mode, steps 1–3 use pre-written content from `demo_content.py` – no
API calls are made.
...
├── demo_content.py           # Pre-written demo output for all three agents
├── results/
│   └── trip_december_2027.md # Sample output (generated by demo mode)
